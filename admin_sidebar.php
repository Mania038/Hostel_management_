...
if (!function_exists('a_active')) {
    function a_active(string $page, string $current): string { return $page === $current ? 'active' : ''; }
}
?>
<aside class="sidebar">
  <div class="sidebar-brand" style="background:linear-gradient(135deg,#b794f4,#63b3ed);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;">
    UniNest <?= $is_super ? 'Admin' : 'Manager' ?>
  </div>
  ...
  <a class="sidebar-item <?= a_active('tasks.php',$current) ?>" href="<?= APP_URL ?>/admin/tasks.php">
  ...
  <a class="sidebar-item <?= a_active('managers.php',$current) ?>" href="<?= APP_URL ?>/admin/managers.php">
    <span class="sidebar-icon">👤</span> Hostel Managers
  </a>
  <?php endif; ?>
  ...
  <div class="sidebar-user">
    <div class="avatar" style="background:linear-gradient(135deg,#b794f4,#63b3ed);">
      <?= strtoupper(substr($_SESSION['admin_name']??'A',0,1).substr(explode(' ',$_SESSION['admin_name']??'AD')[1]??'D',0,1)) ?>
    </div>
    <div class="sidebar-user-info">
      <div class="sidebar-user-name"><?= clean($_SESSION['admin_name'] ?? 'Admin') ?></div>
      <div class="sidebar-user-role">
        <?= $is_super ? 'Super Admin' : 'Block '.$my_block.' Manager' ?>
      </div>
    </div>
  </div>
</aside>